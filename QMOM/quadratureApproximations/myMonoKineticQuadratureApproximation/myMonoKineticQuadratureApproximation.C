...
Foam::myMonoKineticQuadratureApproximation::myMonoKineticQuadratureApproximation
(
    const word& name,
    const fvMesh& mesh,
    const word& support
    ...
    minM1_(readScalar((*this).subDict("residuals").lookup("minM1")))
{
    ...
    //***
    Info<< "Before UTypes: " << UTypes << endl;
    //***
    ...
    forAll(nodes_(), nodei)
    {
        ...
        //***
        Info<< "velocityAbscissae_.BC: " << velocityAbscissae_[nodei].boundaryField().types() << endl;
        //***
...
void Foam::myMonoKineticQuadratureApproximation::interpolateNodes()
{
...
void Foam::myMonoKineticQuadratureApproximation::updateBoundaryVelocities()
{
...
void Foam::myMonoKineticQuadratureApproximation::updateAllQuadrature()
{
    const volScalarField& m0 = moments_[0];
    const volScalarField::Boundary m0Bf = m0.boundaryField();
...
bool Foam::myMonoKineticQuadratureApproximation::updateAllLocalQuadrature
(
    const label celli,
...
void Foam::myMonoKineticQuadratureApproximation::updateVelocities()
{
...
void Foam::myMonoKineticQuadratureApproximation::updateLocalVelocities
(
    const label celli
)
{
...
void Foam::myMonoKineticQuadratureApproximation::updateVelocityMoments()
{
    // obtain bf
    const volScalarField& m0 = moments_[0];
    const volScalarField::Boundary m0Bf = m0.boundaryField();
    ...
    forAll(velocityMoments_, mi)
    {
        ...
        if (mi == 0)
        {
            ...
            {      
                if 
                (
                    //isA<uniformFiexedValueFvPatchVectorField>(U_.boundaryField()[patchi])
                    //isA<uniformFiexedValueFvPatchField>(U_.boundaryField()[patchi])
                    U_.boundaryField().types()[patchi] == "uniformFixedValue"
                )
                {
                    //Info<< "inlet patchi: " << patchi << endl;
                    // Up = m_p * U
                    forAll(m0Bf[patchi], facei)
                    {
                        velocityMoments_[mi].boundaryFieldRef()[patchi][facei] = moments_[mi].boundaryFieldRef()[patchi][facei] * velocityAbscissae_[0].boundaryFieldRef()[patchi][facei];
                    }  
                }
            }
            // ***
...
void Foam::myMonoKineticQuadratureApproximation::updateLocalVelocityMoments
(
    const label celli
)
{
...
void Foam::myMonoKineticQuadratureApproximation::updateAllMoments()
{
    // Update size moments
    updateMoments();
...
void Foam::myMonoKineticQuadratureApproximation::updateAllLocalMoments
(
    const label celli
)
{
