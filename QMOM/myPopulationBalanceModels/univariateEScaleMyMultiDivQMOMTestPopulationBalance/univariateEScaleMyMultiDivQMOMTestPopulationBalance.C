...
#include "univariateEScaleMyMultiDivQMOMTestPopulationBalance.H"
...
{
    defineTypeNameAndDebug(univariateEScaleMyMultiDivQMOMTestPopulationBalance, 0);
    addToRunTimeSelectionTable
    (
        populationBalanceModel,
        univariateEScaleMyMultiDivQMOMTestPopulationBalance,
        dictionary
    );
}
}
...
Foam::PDFTransportModels::populationBalanceModels::univariateEScaleMyMultiDivQMOMTestPopulationBalance
::univariateEScaleMyMultiDivQMOMTestPopulationBalance
(
    const word& name,
...
:
    univariateMyTestPDFTransportModel(name, dict, phi.mesh(), phi, "RPlus"),
...
::realizableCo() const
{
    return univariateMyTestPDFTransportModel::realizableCo();
}
...
*/
        ...
        if(!nodes[0].extended())
        {
            forAll(nodes, pNodeI)
            {
                ...
                volScalarField drift = 2 * dvdtf / (Foam::constant::mathematical::pi);

                forAll(quadrature_.moments()[1], celli)
                {
                    if (quadrature_.moments()[3][celli] >= 1e-8)
                    {
...
::solve()
{
    univariateMyTestPDFTransportModel::solve();
}


const Foam::scalarQuadratureApproximation& Foam::PDFTransportModels::populationBalanceModels::univariateEScaleMyMultiDivQMOMTestPopulationBalance
::quadrature() const
{
    return univariateMyTestPDFTransportModel::quadrature();
}
