...
/*
Foam::tmp<Foam::volScalarField>
Foam::twoPhaseSystem::Kh(const label nodei) const
{
...
/*
    ...
    {
        // *****
        //Info<< "Define AEqns" << endl;
        //Info<<  phase1_->Us(nodei).boundaryField().size() << endl;
        //Info<<  phase1_->Us(nodei).boundaryField().types() << endl;
        //Info<<  phase1_->U().boundaryField().size() << endl;
        //Info<<  phase1_->U().boundaryField().types() << endl;
*/
        /*
        forAll(phase1_->Us(nodei).boundaryField(), i)
        {
            Info<< i << ": " << phase1_->Us(nodei).boundaryField()[i].patch().name() << endl;
            //Info<< i << ": " << phase1_->Us(nodei).boundaryField()[i].patch().type() << endl;
            if
            (
                isA<fixedValueFvPatchVectorField>(phase1_->Us(nodei).boundaryField()[i])
            || isA<slipFvPatchVectorField>(phase1_->Us(nodei).boundaryField()[i])
            || isA<partialSlipFvPatchVectorField>(phase1_->Us(nodei).boundaryField()[i])
            )
            {
                Info<< i << phase1_->Us(nodei).boundaryField()[i].patch().name() << endl;
            }
        ...
        */
        ...
        (
            ...
            (
                phase1_->U(),
                phase1_->U().dimensions()*dimDensity*dimVol/dimTime
            )
        );
        */
 /*       
...
*/
    ...
    /*
    PtrList<fvVectorMatrix> myAEqns(getAEqns());
    for (label nodei = 0; nodei < nNodes_; nodei++)
    {
        Info<< "AEqns." << nodei << ": " << endl
            << "D: " << average(AEqns[nodei].D()) << endl
            << "source: " << average(AEqns[nodei].source()) << endl;

        Info<< "myAEqns." << nodei << ": " << endl
            << "D: " << average(myAEqns[nodei].D()) << endl
            << "source: " << average(myAEqns[nodei].source()) << endl;
    }
    */
    // ********
    PtrList<fvVectorMatrix> myAEqns(getAEqns());
    //phase1_->averageTransport(AEqns);
    //***
    Info<< "after getting AEqns" << endl;
    //Info<< myAEqns << endl;
    //***
    phase1_->averageTransport(myAEqns);
    phase1_->correct();
...
void Foam::twoPhaseSystem::correctEnergyTransport()
{
    phase1_->correctEnergyTransport();
    phase2_->correctEnergyTransport();
}
...
const Foam::HashTable<Foam::autoPtr<Foam::blendingMethod>>& Foam::twoPhaseSystem::blendingMethods() const
{
    return blendingMethods_;
}
...
Foam::tmp<Foam::volScalarField> Foam::twoPhaseSystem::dmdt() const
{
    return volScalarField::New
    (
        IOobject::groupName("dmdt", pair_->name()),
        this->mesh_,
...
Foam::twoPhaseSystem::massTransfer() const
{
    // Create a mass transfer matrix for each species of each phase
    autoPtr<massTransferTable> eqnsPtr
    (
        new twoPhaseSystem::massTransferTable()
    );

    massTransferTable& eqns = eqnsPtr();
/*
...
*/
    const PtrList<volScalarField>& Yi = phase1_->Y();
    ...
    const PtrList<volScalarField>& Yj = phase2_->Y();

    forAll(Yj, j)
    {
        Info<< "Set for" << Yj[j].name() << endl;
        eqns.set
        (
            Yj[j].name(),
            new fvScalarMatrix(Yj[j], dimMass/dimTime)
        );
    }
...
void Foam::twoPhaseSystem::correctKinematics()
{
    /*
    ...
    */
    phase1_->correctKinematics();
    phase2_->correctKinematics();
}


Foam::PtrList<Foam::fvVectorMatrix> 
Foam::twoPhaseSystem::getAEqns() const
{
    PtrList<fvVectorMatrix> AEqns(nNodes_);
    //***
    //Info<< "nNodes_: " << nNodes_ << endl;
    //***
    ...
    for (label nodei = 0; nodei < nNodes_; nodei++)
    {
        ...
        //  forces
        //*****
        //Info<< "before AEqns over in twoPhaseSystem" << endl;
        //Info<< "phase1->Us_: " << phase1_->Us(nodei) << endl;
        //Info<< "after Us" << endl;
        //*****
        AEqns.set
        (
        ...
        //*****
        //Info<< "Define AEqns over in twoPhaseSystem" << endl;
        //*****
        const volScalarField& p(mesh_.lookupObject<volScalarField>("p"));
        volScalarField alphaRhoi(phase1_->alphas(nodei)*phase1_->rho());
