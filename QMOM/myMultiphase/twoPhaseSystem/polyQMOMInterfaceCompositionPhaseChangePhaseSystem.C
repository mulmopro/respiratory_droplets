...
Foam::polyQMOMInterfaceCompositionPhaseChangePhaseSystem<BasePhaseSystem>::iDmdt
() const
{
...
template<class BasePhaseSystem>
Foam::polyQMOMInterfaceCompositionPhaseChangePhaseSystem<BasePhaseSystem>::
polyQMOMInterfaceCompositionPhaseChangePhaseSystem
(
    const fvMesh& mesh, 
...
template<class BasePhaseSystem>
Foam::tmp<Foam::volScalarField>
Foam::polyQMOMInterfaceCompositionPhaseChangePhaseSystem<BasePhaseSystem>::dmdt
() const
{
...
Foam::polyQMOMInterfaceCompositionPhaseChangePhaseSystem<BasePhaseSystem>::dmdts() const
{
...
template<class BasePhaseSystem>
void Foam::polyQMOMInterfaceCompositionPhaseChangePhaseSystem<BasePhaseSystem>::
correct()
{
...
template<class BasePhaseSystem>
bool Foam::polyQMOMInterfaceCompositionPhaseChangePhaseSystem<BasePhaseSystem>::read()
{
...
Km2(const label nodei) const
{
    ...
    //Info<< "in km2(nodei)" << endl;
    tmp<volScalarField> tK(massTransferModel2_->K(nodei, 0));
    //volScalarField& K = tK.ref();

    //volScalarField K(massTransferModel2_->K(nodei, 0));
    ...
    (
        ...
        (
            ...
            this->mesh(),
            dimensionedScalar(dimMass/dimTime/dimVolume)
        )
    );
    ...
    {
        const volScalarField KD(tK.ref()*interfaceCompositionModel2_->D(member));

        const volScalarField Yf(interfaceCompositionModel2_->Yf(member, this->Tf_));
        mDot.ref() += this->phase2().rho()*KD*(Yf - this->phase2().Y(member));
    }

    dimensionedScalar dMSmall("dMSmall", dimMass/dimTime/dimVolume, VSMALL);
    //Info<< "in km2(nodei), before min max of K" << endl;
    tK.ref() = tK.ref() * min(
        1.0,
        max(this->phase1().dotMsLim(nodei), dMSmall) / max(mDot.ref(), dMSmall)
    );

    
    //Info<< "in km2(nodei), after min max of K" << endl;
    //Info<< "K.patch: " << K.boundaryField().types() << endl;

    return tK;
    //return massTransferModel2_->K(nodei, 0) * min(1.0, 2.0);
}


template<class BasePhaseSystem>
Foam::tmp<Foam::volScalarField>
Foam::polyQMOMInterfaceCompositionPhaseChangePhaseSystem<BasePhaseSystem>::Km2() const
{
    ...
    (
        ...
        (
            ...
            this->mesh(),
            dimensionedScalar("Km", inv(dimArea), 0.0)
            /*
            dimensionedScalar
            (
                IOobject::groupName("Km", this->phase2().name()),
    ...
    //Info<< "in km2(nodei), before sum of K" << endl;
    //Info<< "K.patch: " << tKm->boundaryField().types() << endl;
    for (label nodei = 0; nodei < this->phase1().nNodes(); nodei++)
    {
        tKm.ref() += Km2(nodei);
    }
    //Info<< "in km2(nodei), after sum of K" << endl;
    return tKm;
}
...
/*
template<class BasePhaseSystem>
Foam::HashPtrTable<Foam::volScalarField> Foam::polyQMOMInterfaceCompositionPhaseChangePhaseSystem<BasePhaseSystem>::dmidtfs() const
{
...
/*
template<class BasePhaseSystem>
void Foam::polyQMOMInterfaceCompositionPhaseChangePhaseSystem<BasePhaseSystem>::averageTransport()
{
    const phaseModel& phase1 = this->phase1();
    const phaseModel& phase2 = this->phase2();
...
Foam::polyQMOMInterfaceCompositionPhaseChangePhaseSystem<BasePhaseSystem>::getAEqns() const
{
    // ***
